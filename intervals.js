...
function intervalConstruction(arr) {
  try {
    ...
    if (direction === 'dsc') {
      ...
      const semitoneName = findSemitoneNameDSC(
        newNotes,
        interval.semitone,
        noteEnd.name,
        semitone
    ...
    const noteEnd = newNotes[interval.degrees - 1];
    const semitoneName = findSemitoneNameASC(
      newNotes,
      interval.semitone,
      noteEnd.name,
      semitone
...
function intervalIdentification(arr) {
  try {
    ...
    if (direction === 'dsc') {
      const reversNotes = NOTES.slice().reverse();
      const startIndex = reversNotes.findIndex(
        (item) => item.name === noteStart.name
      );
      const newArr = getNewNotesArr(reversNotes, startIndex);
      const degree = findDegree(newArr, noteEnd);
      const interval = findSemitoneIntervalDSC(
        newArr,
        noteEnd,
        posStart,
        posEnd
...
const findNameInterval = (degree, semitone) => {
  const name = INTERVALS.find((item) => {
    if (item.degrees === degree && item.semitone === semitone) {
      return item;
    }
  });
  return name.name;
